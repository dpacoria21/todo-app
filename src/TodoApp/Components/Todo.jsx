...
export const Todo = ({todo, handleDeleteTodo, handleCompleteTodo, handleActiveTodo, handlePendingTodo}) => {
    ...
    return (
        <div onDoubleClick={disabled} className={`relative ${state} h-full ${enabled ? 'opacity-50':''} py-2 flex flex-col sm:flex-row gap-1 cursor-pointer items-center justify-between transition-all duration-300 px-5 rounded-sm hover:scale-110`}>
            {
            ...
            <div className={`flex ${enabled && 'pointer-events-none'} justify-center flex-wrap items-center gap-2`}>
                <button onClick={onCompleteTodo} className="cursor-pointer transition-all duration-300 hover:scale-110">
                ...
                <button onClick={onPendingTodo} className="cursor-pointer transition-all duration-300 hover:scale-110 bg-sky-100 rounded-full h-6 w-6 flex justify-center items-center">
                ...
                <button onClick={onActiveTodo} className="cursor-pointer transition-all duration-300 hover:scale-110">
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" className="bi bi-arrow-counterclockwise fill-sky-600 p-1 bg-sky-100 rounded-full" viewBox="0 0 16 16">
                        <path fillRule="evenodd" d="M8 3a5 5 0 1 1-4.546 2.914.5.5 0 0 0-.908-.417A6 6 0 1 0 8 2v1z"/>
                        <path d="M8 4.466V.534a.25.25 0 0 0-.41-.192L5.23 2.308a.25.25 0 0 0 0 .384l2.36 1.966A.25.25 0 0 0 8 4.466z"/>
                    </svg>
                </button>
                <button onClick={onDeleteTodo} className="cursor-pointer transition-all duration-300 hover:scale-110">
